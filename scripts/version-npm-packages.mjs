...
function bumpVersion(version, bumpType) {
	...
	return bumpType === "patch" ? `${major}.${minor}.${patch + 1}` : `${major}.${minor + 1}.0`;
}
...
if (agentPackage.version !== codingAgentPackage.version) {
	console.error(`Expected agent and coding-agent to share a version, got ${agentPackage.version} and ${codingAgentPackage.version}`);
	process.exit(1);
}

...
describe("detectInstallMethod", () => {
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@mupt-ai/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@mupt-ai/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@mupt-ai/pi-coding-agent")).toContain(

...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@mariozechner/pi-ai`, `@mupt-ai/pi-agent-core`, `@mupt-ai/pi-coding-agent`, `@mariozechner/pi-tui`, `@sinclair/typebox`.

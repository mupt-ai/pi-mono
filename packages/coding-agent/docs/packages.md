...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@earendil-works/pi-ai`, `@mupt-ai/pi-agent-core`, `@mupt-ai/pi-coding-agent`, `@earendil-works/pi-tui`, `typebox`.
